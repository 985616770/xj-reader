    ...
    .book-speak-mask-wrapper(
      ...
      transition(name="popup-slide-up")
        .book-speak-mask-card-wrapper(
          ...
          .pulldown-icon-wrapper(@click="hide")
            span.icon-pull_down
          .card-title-wrapper
            .icon-speak-wrapper
              span.icon-speak
            .speak-title-wrapper
              span.speak-title {{ $t('speak.voice') }}
            .read-fulltext-wrapper
              span.read-fulltext {{ $t('speak.read') }}
          .card-section-title-wrapper
            .card-section-title-text {{ title }}
            .card-section-sub-title-text(v-if="currentSectionIndex")   {{ currentSectionIndex }} / {{ currentSectionTotal }}
          .setting-progress
            .progress-wrapper
              input.progress(
                type="range"
              ...
              .progress-text
                .progress-text-current {{ playInfo ? playInfo.currentMinute : '00' }}:{{ playInfo ? playInfo.currentSecond : '00' }}
                .progress-text-left -{{ playInfo ? playInfo.leftMinute : '00' }}:{{ playInfo ? playInfo.leftSecond : '00' }}
          .playing-wrapper
            .icon-settings-wrapper
              span.icon-settings
              .settings-text {{ $t('speak.settings') }}
            span.icon-play_backward(
            ...
            span.icon-play_forward(
              :class="{ 'not-use': currentSectionIndex >= currentSectionTotal }"
              @click.stop.prevent="next"
            )
            .icon-clock-wrapper
              .icon-clock
              .clock-text {{ $t('speak.timing') }}
          .read-apply-wrapper  {{ $t('speak.apply') }}
          .read-title-wrapper
            span.line
            .read-title-text {{ $t('speak.current') }}
            span.line
          .book-wrapper(ref="bookWrapper")
            #book-read
</template>
...
export default {
  ...
  methods: {
    ...
    refresh() {
      this.readHeight =
        window.innerHeight * 0.9 - realPx(40) - realPx(54) - realPx(46) - realPx(48) - realPx(60) - realPx(44)
    ...
    .setting-progress {
      ...
      .progress-wrapper {
        ...
        .progress {
          ...
          height: px2rem(2);
          background: -webkit-linear-gradient($color-blue, $color-blue) no-repeat, rgba(74, 171, 255, 0.2);
